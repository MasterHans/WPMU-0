<?php
function motivation_all_published_post(){
    $data = array();
    $args = array(
        'post_type'     => 'product',
        'post_status'   => 'publish',
...
add_action('init', 'motivation_king_slider_data', 99);

function motivation_king_slider_data(){

    global $kc;
    if( function_exists('kc_add_map') ){
        $kc->add_map(array(
            'motivation_kings_slider'    => array(
                'name'        => 'Motivation Slider',
                ...
                'params'      => array(
                                        array(
                                            ...
                                            'label'         => 'Field Label',
                                            'type'          => 'multiple',
                                            'options'       => motivation_all_published_post(),
...
function motivation_king_slider_shortcodes($atts, $content = null){
    ...
    foreach ($data as $key => $value) {
        ...
        if( isset($id->ID) ){
            if( $id->ID ){
                ...
                //Get motivation set raised
                $total_raised = MOTIVATIONCF()->totalFundRaisedByCampaign($id);
                if ($total_raised){
                    $raised = $total_raised;
                }
                ...
                $output .= '<li>';
                if(function_exists('wpneo_crowdfunding_price')){
                    $output .= '<h3 class="amount">'.wpneo_crowdfunding_price($total_raised).'</h3>';
                }
                ...
                $output .= '<a href="'.get_permalink( $id ).'" class="btn btn--green btn--large">'.__('View Project','crowdfunding').'</a>';
...
add_shortcode('motivation_kings_slider', 'motivation_king_slider_shortcodes');
